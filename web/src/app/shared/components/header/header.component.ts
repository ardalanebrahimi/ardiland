      ...
      <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-6">
          ...
          <button
            ...
            [class]="themeService.dark() ? 'hover:bg-gray-800' : 'hover:bg-gray-100'"
            [attr.aria-label]="themeService.dark() ? 'Switch to light mode' : 'Switch to dark mode'"
          >
            @if (themeService.dark()) {
              <!-- Sun icon -->
              <svg xmlns="http://www.w3.org/2000/svg" class="h-[18px] w-[18px] text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
            } @else {
              <!-- Moon icon -->
              <svg xmlns="http://www.w3.org/2000/svg" class="h-[18px] w-[18px] text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex md:hidden items-center gap-2">
          ...
          >
            @if (mobileOpen()) {
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" [class]="themeService.dark() ? 'text-white' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
